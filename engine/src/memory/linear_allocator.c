...
void linear_allocator_create(u64 total_size, void* memory, linear_allocator* out_allocator)
{
    if(out_allocator)
    {
        ...
        if(memory)
            ...
            out_allocator->memory = veallocate(total_size, MEMORY_TAG_LINEAR_ALLOCATOR);
    }
}
...
void linear_allocator_destroy(linear_allocator* allocator)
{
    if(allocator)
    {
        ...
        if(allocator->owns_memory && allocator->memory)
            vefree(allocator->memory, allocator->total_size, MEMORY_TAG_LINEAR_ALLOCATOR);;
...
void* linear_allocator_allocate(linear_allocator* allocator, u64 size)
{
    if(allocator && allocator->memory)
    {
        if(allocator->allocated + size > allocator->total_size)
        {
            ...
            VEERROR("linear_allocator_allocate - Tried to allocate %lluB, only %lluB remaining", size, remaining);
            return 0;
        }   
    ...
    VEERROR("linear_allocator_allocate - provided allocator not initialized.");
    return 0;
}

void linear_allocator_frea_all(linear_allocator* allocator)
{
    if(allocator && allocator->memory)
    {
        allocator->allocated = 0;
        vezero_memory(allocator->memory, allocator->total_size);
    }
}