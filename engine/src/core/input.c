...
void input_system_initialize(u64* memory_requirement, void* state) {
    *memory_requirement = sizeof(input_state);
    ...
    vezero_memory(state, sizeof(input_state));
    state_ptr = state;

    VEINFO("Input subsystem initialized.");
}
...
void input_update(f64 delta_time)
{
    ...
    vecopy_memory(&state_ptr->keyboard_previous, &state_ptr->keyboard_current, sizeof(keyboard_state));
    vecopy_memory(&state_ptr->mouse_previous, &state_ptr->mouse_current, sizeof(mouse_state));
}

void input_process_key(keys key, b8 pressed)
{
    if(key == KEY_LALT) {
        VEINFO("Left alt pressed.");
    }
    else if(key == KEY_RALT) {
        VEINFO("Right alt pressed.");
    }
    
    if(key == KEY_LCONTROL) {
        VEINFO("Left control pressed.");
    }
    else if(key == KEY_RCONTROL) {
        VEINFO("Right control pressed.");
    }

    if(key == KEY_LSHIFT) {
        VEINFO("Left shift pressed.");
    }
    else if(key == KEY_RSHIFT) {
        VEINFO("Right shift pressed.");
    }
    
    // Only execute this if the state actually changed.
