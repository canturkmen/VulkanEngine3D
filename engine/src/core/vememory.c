...
char* get_memory_usage_str()
{
    ...
    for(u32 i = 0; i < MEMORY_TAG_MAX_TAGS; ++i)
    {
        char unit[4] = "XiB";
        float amount = 1.0f;
        if(state_ptr->stats.tagged_allocation[i] >= gib) 
        {
            unit[0] = 'G';
            amount = state_ptr->stats.tagged_allocation[i] / (float)gib;
        }
        else if (state_ptr->stats.tagged_allocation[i] >= mib)
        {
            unit[0] = 'M';
            amount = state_ptr->stats.tagged_allocation[i] / (float)mib;
        }
        else if (state_ptr->stats.tagged_allocation[i] >= kib)
        {
            unit[0] = 'K';
            amount = state_ptr->stats.tagged_allocation[i] / (float)kib;
        }
        else
        {
            unit[0] = 'B';
            unit[1] = 0;
            amount = (float)state_ptr->stats.tagged_allocation[i];
        }
