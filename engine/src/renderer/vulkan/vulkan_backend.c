...
{
    // Create a host-visible staging staging buffer to upload to. Mark it as the source of the transfer.
    VkBufferUsageFlags flags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
    vulkan_buffer staging;
    vulkan_buffer_create(context, size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, flags, true, &staging);

    // Load the data into staging buffer.
...
{   
    ...
    vezero_memory(verts, sizeof(vertex_3d) * vert_count);
...
b8 vulkan_renderer_backend_begin_frame(struct renderer_backend* backend, f32 delta_time)
{
    ...
    if(context.recreating_swapchain)
    {
        ...
        {
            VEERROR("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle (1) failed: '%s'", vulkan_result_string(result, true));
            return false;
        }
        VEINFO("Recreating swapchain, booting.");
        return false;
    }
 
    // Check if framebuffer has been resized. If so, a new swapchain must be created.
    if(context.framebuffer_size_generation != context.framebuffer_size_last_generation)
    {
        VkResult result = vkDeviceWaitIdle(device->logical_device);
        if(!vulkan_result_is_success(result))
        {
            VEERROR("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle (2) failed: '%s'", vulkan_result_string(result, true));
            return false;
        }
    ...
    {
        VEWARN("In-Flight fence wait failure!");
        return false;
    }
...
b8 vulkan_renderer_backend_end_frame(struct renderer_backend* backend, f32 delta_time)
{
    ...
    // The semaphore(s) to signaled when the queue is complete.
    ...
    {   
        VEERROR("vkQueueSubmit failed with result: %s", vulkan_result_string(result, true));
        return false;
    }

    vulkan_command_buffer_submitted(command_buffer);
...
i32 find_memory_index(u32 type_filter, u32 property_flags)
{
    ...
    VEWARN("Unable to find suitable memory type!");
    return -1;
}
...
void create_command_buffers(renderer_backend* backend)
{
    if(!context.graphics_command_buffers)
    {
        ...
        for(u32 i = 0; i < context.swapchain.image_count; ++i)
            vezero_memory(&context.graphics_command_buffers[i], sizeof(vulkan_command_buffer));
    }
...
b8 recreate_swapchain(renderer_backend* backend)
{
    ...
    if(context.recreating_swapchain)
    {
        VEDEBUG("recreate_swapchain called when already recreating. Booting.");
        return false;
    }
    ...
    {
        VEDEBUG("recreate_swapchain called when window is < 1 in a dimension. Booting.");
        return false;
    }
...
void vulkan_renderer_create_texture(const char* name, b8 auto_release, i32 width, i32 height, i32 channel_count, const u8* pixels, b8 has_transparency, texture* out_texture)
{
    ...
    out_texture->internal_data = (vulkan_texture_data*)veallocate(sizeof(vulkan_texture_data), MEMORY_TAG_TEXTURE);
    ...
    {
        VEERROR("Error creating texture sampler: %s", vulkan_result_string(result, true));
        return;
    }
...
void vulkan_renderer_destroy_texture(texture* texture) 
{
    ...
    vezero_memory(&data->image, sizeof(vulkan_image));
    vkDestroySampler(context.device.logical_device, data->sampler, context.allocator);
    data->sampler = 0;

    vefree(texture->internal_data, sizeof(vulkan_texture_data), MEMORY_TAG_TEXTURE);
    vezero_memory(texture, sizeof(struct texture));
}