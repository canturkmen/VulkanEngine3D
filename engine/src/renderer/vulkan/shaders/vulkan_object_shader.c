...
b8 vulkan_object_shader_create(vulkan_context* context, vulkan_object_shader* out_shader) 
{
    ...
    for (u32 i = 0; i < OBJECT_SHADER_STAGE_COUNT; ++i) 
    {
        ...
        {
            VEERROR("Unable to create %s shader module for '%s'.", stage_type_strs[i], BUILTIN_SHADER_NAME_OBJECT);
            return false;
        }
    }
    ...
    vezero_memory(stages_create_infos, sizeof(stages_create_infos));
    for(u32 i = 0; i < OBJECT_SHADER_STAGE_COUNT; ++i)
    {
        stages_create_infos[i].sType = out_shader->stages[i].shader_stage_create_info.sType;
        stages_create_infos[i] = out_shader->stages[i].shader_stage_create_info;
    }
    ...
    if(!vulkan_graphics_pipeline_create(
            ...
            &out_shader->pipeline))
    {
        VEERROR("Failed to load graphics pipeline for object shader.");
        return false;
    }
    ...
    {
        VEERROR("Vulkan buffer creation failed for object shader.");
        return false;
    }
