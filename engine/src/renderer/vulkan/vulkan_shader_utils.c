    ...
    vulkan_shader_stage* shader_stages) 
{
    ...
    vezero_memory(&shader_stages[stage_index].create_info, sizeof(VkShaderModuleCreateInfo));
    ...
    {
        VEERROR("Unable to read shader module: %s.", file_name);
        return false;
    }
    ...
    {
        VEERROR("Unable to binary read shader module: %s.", file_name);
        return false;
    }
    ...
    // Shader stage info
    vezero_memory(&shader_stages[stage_index].shader_stage_create_info, sizeof(VkPipelineShaderStageCreateInfo));
    ...
    if (file_buffer) 
    {
        vefree(file_buffer, sizeof(u8) * size, MEMORY_TAG_STRING);
        file_buffer = 0;
    }
    return true;
}