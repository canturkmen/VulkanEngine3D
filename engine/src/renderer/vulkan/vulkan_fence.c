    ...
    if(!fence->is_signaled)
    {
        ...
        switch (result)
        {
            case VK_SUCCESS:
                ...
                return TRUE;
            case VK_TIMEOUT:
                VEWARN("vk_fence_wait - Timed out.");
                break;
            case VK_ERROR_DEVICE_LOST:
                VEERROR("vk_fence_wait - VK_ERROR_DEVICE_LOST.");
                break;
            case VK_ERROR_OUT_OF_HOST_MEMORY:
                VEERROR("vk_fence_wait - VK_ERROR_OUT_OF_HOST_MEMORY.");
                break;
            case VK_ERROR_OUT_OF_DEVICE_MEMORY:
                VEERROR("vk_fence_wait - VK_ERROR_OUT_OF_DEVICE_MEMORY.");
                break;
            default:
                VEERROR("vk_fence_wait - An unknown error has occured.");
                break;
        }
    }
    else
