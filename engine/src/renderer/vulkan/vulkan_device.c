...
b8 vulkan_device_create(vulkan_context* context)
{
    ...
    VEINFO("Creating logical device...");
    ...
    VEINFO("Graphics command pool created.");
    return true;
}
...
void vulkan_device_destroy(vulkan_context* context)
{
    ...
    VEINFO("Destroying command pools...");
    ...
    VEINFO("Destroying logical device...");
    ...
    VEINFO("Releasing physical device resoures...");
    context->device.physical_device = 0;

    if(context->device.swapchain_support.formats)
    {
        vefree(context->device.swapchain_support.formats, 
    ...
    {
        vefree(context->device.swapchain_support.present_modes, 
    ...
    vezero_memory(&context->device.swapchain_support.capabilities, sizeof(context->device.swapchain_support.capabilities));
    ...
    vulkan_swapchain_support_info* out_support_info)
{
    ...
    if(out_support_info->format_count != 0)
    {
        if(!out_support_info->formats)
            out_support_info->formats = veallocate(sizeof(VkSurfaceFormatKHR) * out_support_info->format_count, MEMORY_TAG_RENDERER);
...
b8 select_physical_device(vulkan_context* context)
{
    ...
    {
        VEFATAL("No devices which support Vulkan were found.");
        return false;
    }
    ...
    for(u32 i = 0; i < physical_device_count; ++i)
    {
        ...
        if(result)
        {
            VEINFO("Selected device: '%s'.", properties.deviceName);
            // GPU type, etc.
            switch (properties.deviceType)
            {
                default:
                case VK_PHYSICAL_DEVICE_TYPE_OTHER:
                    VEINFO("GPU type is unknown.")
                    break;
                case VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU:
                    VEINFO("GPU type is Integrated.")
                    break;
                case VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU:
                    VEINFO("GPU type is Descrete.")
                    break;
                case VK_PHYSICAL_DEVICE_TYPE_VIRTUAL_GPU:
                    VEINFO("GPU type is Virtual.")
                    break;
                case VK_PHYSICAL_DEVICE_TYPE_CPU:
                    VEINFO("GPU type is CPU.")
                    break;
            }

            VEINFO("GPU Driver version: %d.%d.%d",
                VK_VERSION_MAJOR(properties.driverVersion),
                VK_VERSION_MINOR(properties.driverVersion),
                VK_VERSION_PATCH(properties.driverVersion));

            // Vulkan API version.
            VEINFO("Vulkan API version: %d.%d.%d",
            ...
            context->device.present_queue_index = queue_info.graphics_family_index;
            context->device.transfer_queue_index = queue_info.graphics_family_index;
    ...
    {
        VEERROR("No physical device were found which meet the requirements.");
        return false;
    }

    VEINFO("Physical device was selected.");
    return true;
} 
    ...
    vulkan_swapchain_support_info* out_swapchain_support)
{
    ...
    if(requirements->discrete_gpu) 
    {
        ...
        {
            VEINFO("Device is not a discrete GPU, and one is required. Skipping.");
            return false;
        }
    }
    ...
    VEINFO("Graphics | Present | Compute | Transfer | Name");
    u8 min_transfer_score = 255;
    for(u32 i = 0; i < queue_family_count; ++i)
    {
        u8 current_transfe_score = 0;

        // Graphics queue?
        if(queue_families[i].queueFlags & VK_QUEUE_GRAPHICS_BIT)
        {
            out_queue_family_info->graphics_family_index = i;
            ++current_transfe_score;
        }
        ...
        if(queue_families[i].queueFlags & VK_QUEUE_TRANSFER_BIT)
        {
            ...
            if(current_transfe_score <= min_transfer_score)
    ...
    (!requirements->transfer || (requirements->transfer && out_queue_family_info->transfer_family_index != -1)))
    {
        ...
        if(out_swapchain_support->format_count < 1 || out_swapchain_support->present_mode_count < 1)
        {
            if(out_swapchain_support->formats)
                vefree(out_swapchain_support->formats, sizeof(VkSurfaceFormatKHR) * out_swapchain_support->format_count, MEMORY_TAG_RENDERER);
            
            if(out_swapchain_support->present_modes)
                vefree(out_swapchain_support->present_modes, sizeof(VkPresentModeKHR) * out_swapchain_support->present_mode_count, MEMORY_TAG_RENDERER);
            
            VEINFO("Required swapchain support not present, skipping device.")
            return false;
        }
        ...
        if(requirements->device_extension_names)
        {
            ...
            {
                available_extensions = veallocate(sizeof(VkExtensionProperties) * available_extension_count, MEMORY_TAG_RENDERER);
                VK_CHECK(vkEnumerateDeviceExtensionProperties(
                    device,
                    0,
                    &available_extension_count,
                ...
                for(u32 i = 0; i < required_extension_count; ++i)
                {
                    ...
                    if(!found)
                    {
                        VEINFO("Required extension not found: '%s', skipping device.", requirements->device_extension_names[i]);
                        vefree(available_extensions, sizeof(VkExtensionProperties) * available_extension_count, MEMORY_TAG_RENDERER);
                        return false;
                    }
                }
            }
        ...
        {
            VEINFO("Device does not support samplerAnisotropy, skipping!");
            return false;
        }
