...
#include <X11/Xlib.h>   // sudo apt-get install libxkbcommon-x11-dev    
#include <sys/time.h>
...
b8 platform_system_startup(u64* memory_requirement, void* state, const char* application_name, i32 x, i32 y, i32 width, i32 height)
{
    *memory_requirement = sizeof(platform_state);
    ...
    state_ptr->connection = XGetXCBConnection(state->display);
    ...
    const struct xcb_setup_t *setup = xcb_setup(state_ptr->connection);
    
    // Loop through screens using iterator.
    xcb_screen_iterator it = xcb_setup_roots_iterator(setup);
    int screen_p = 0;
...
b8 platform_create_vulkan_surface(platform_state* plat_state, vulkan_context* context)
{
    ...
    VkXcbSurfaceCreationInfoKHR create_info = {VK_STRUCTURE_TYPE_XCB_SURFACE_CREATE_INFO_KHR};
    create_info.connection = state_ptr->connection;
    create_info.window = state_ptr->window;

    VkResult result = vkCreateXcbSurfaceKHR(context->instance, &create_info, context->allocator, &state_ptr->surface);  
