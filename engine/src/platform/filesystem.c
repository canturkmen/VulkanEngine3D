...
b8 filesystem_open(const char* path, file_modes mode, b8 binary, file_handle* out_handle)
{
    ...
    {
        VEERROR("Invalid mode passed while trying to open file: '%s'", path);
        return false;
    }
    ...
    if(!file)
    {
        VEERROR("Error opening file: '%s'", path);
        return false;
    }
...
b8 filesystem_read_line(file_handle* handle, char** line_buf)
{
    if(handle->handle)
    {
        ...
        if(fgets(buffer, 32000, (FILE*)handle->handle) != 0)
        {
            ...
            *line_buf = veallocate((sizeof(char) * length) + 1, MEMORY_TAG_STRING);
...
b8 filesystem_read_bytes(file_handle* handle, u64 data_size, void* out_data, u64* out_bytes_read)
{
...
b8 filesystem_read_all_bytes(file_handle* handle, u8** out_bytes, u64* out_bytes_read)
{
    if(handle->handle)
    {
        ...
        *out_bytes = veallocate(sizeof(u8) * size, MEMORY_TAG_STRING);
