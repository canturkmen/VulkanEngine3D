 ...
 */
VEAPI b8 filesystem_exists(const char* path);
 ...
 */
VEAPI b8 filesystem_open(const char* path, file_modes mode, b8 binary, file_handle* out_handle);
 ...
 */
VEAPI void filesystem_close(file_handle* handle);
 ...
 */
VEAPI b8 filesystem_read_line(file_handle* handle, char** line_buf);
 ...
 * @returns True if successful; otherwise false.
 */
VEAPI b8 filesystem_read_bytes(file_handle* handle, u64 data_size, void* out_data, u64* out_bytes_read);
 ...
 * @returns True if successful; otherwise false.
 */
VEAPI b8 filesystem_read_all_bytes(file_handle* handle, u8** out_bytes, u64* out_bytes_read);
 ...
 * @returns True if successful; otherwise false.
 */
VEAPI b8 filesystem_write(file_handle* handle, u64 data_size, const void* data, u64* out_bytes_written);