...
#include "core/vememory.h"
 ...
 void* _darray_create(u64 length, u64 stride)
 {
    ...
    u64* new_array = veallocate(header_size + array_size, MEMORY_TAG_DARRAY);
    veset_memory(new_array, 0, header_size + array_size);
...
void _darray_destroy(void* array)
{
    ...
    vefree(header, total_size, MEMORY_TAG_DARRAY);
}
...
void _darray_pop(void* array, void* dest)
{
    ...
    vecopy_memory(dest, (void*)addr, stride);
    _darray_field_set(array, DARRAY_LENGTH, length - 1);
}
...
void* _darray_pop_at(void* array, u64 index, void* dest)
{
    ...
    if(index >= length)
    {
        VEERROR("Index outside the bounds of this array! Length: %i, index: %index", length, index);
        return array;
    }

    u64 addr = (u64)array;
    vecopy_memory(dest, (void*)(addr + (index * stride)), stride);
...
void* _darray_insert_at(void* array, u64 index, void* value_ptr)
{
    ...
    vecopy_memory((void*)(addr + (index * stride)), value_ptr, stride);
