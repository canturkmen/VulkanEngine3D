...
void test_manager_run_tests()
{
    ...
    for(u32 i = 0; i < count; ++i)
    {
        ...
        {
            VEWARN("[SKIPPED]: %s", tests[i].desc);
            ++skipped;
        }
        else
        {
            VEERROR("[FAILED]: %s", tests[i].desc);
            ++failed;
        }
        ...
        VEINFO("Executed %d of %d (skipped %d) %s (%.6f sec / %.6f sec total", i + 1, count, skipped, status, test_time.elapsed_time, total_time.elapsed_time);
    }

    clock_stop(&total_time);
    VEINFO("Results: %d passed, %d failed, %d skipped.", passed, failed, skipped);
}